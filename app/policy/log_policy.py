...
from app.policy.risk_policy import contains_risky_signal, match_risk_rules
from app.schemas.log_summary import LogSummaryRequest, LogSummaryResponse
from app.utils.text import first_sentence, normalize_search_text, pick_matching_lines, split_lines, unique_list
...
def summarize_log_policy(request: LogSummaryRequest) -> LogSummaryResponse:
    ...
    matches = match_risk_rules(merged)
    for keyword in IMPORTANT_LINE_KEYWORDS:
        if normalize_search_text(keyword) in merged:
            matches.append(
                matched_rule(
                    rule_name="important_log_keyword",
                    rule_type="keyword",
                    matched_value=keyword,
                    effect="highlight:important_line",
                )
            )
    ...
    return LogSummaryResponse(
        ...
        next_step=next_step,
        decision_trace=build_decision_trace(matches, risky=risky, strong=needs_codex or recommended_tool == "gemini"),
    )
